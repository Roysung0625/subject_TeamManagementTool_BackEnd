...
Rails.application.routes.draw do
  ...
  namespace :api do
    ...
    resources :tasks, only: %i[create update destroy show] do
      # 特定社員の本日のタスク取得
      get    'list/:employee_id/today', to: 'tasks#employee_today', on: :collection
      # 特定社員の全タスク取得（フィルタリング: ?category=&status=）
      get    'list/:employee_id', to: 'tasks#index_by_employee', on: :collection
      # 特定チームの全タスク取得
      get    'list/:team_id', to: 'tasks#index_by_team', on: :collection
      # 特定チームの本日のタスク取得
      get    'list/:team_id/today', to: 'tasks#team_today', on: :collection
    end
